...
export default function CreateTab(props) {
  ...
  const [tire, setTire] = React.useState({
    number: '',
    width: '155',
    height: '20',
    size: '13',
  ...
  const updateTire = React.useCallback(
    (state) => {
      setTire({ ...tire, ...state });
    },
    [tire],
  ...
  return (
    <>
      <Typography component="div" className={classes.main}>
        <NumberField
          value={tire.number}
          onChange={(number) => updateTire({ number })}
        />
        <WidthField
          value={tire.width}
          onChange={(width) => updateTire({ width })}
        />
        <HeightField
          value={tire.height}
          onChange={(height) => updateTire({ height })}
        />
        <SizeField
          value={tire.size}
          onChange={(size) => updateTire({ size })}
        />
        <BrandField
          value={tire.brand}
          onChange={(brand) => updateTire({ brand })}
        />
        <ManufacturerField
          value={tire.manufacturer}
          onChange={(manufacturer) => updateTire({ manufacturer })}
        />
        <YearField
          value={tire.year}
          onChange={(year) => updateTire({ year })}
        />
        <PriceField
          value={tire.price}
          onChange={(price) => updateTire({ price })}
        />
        <LocationField
          value={tire.location}
        ...
        <RemarkField
          value={tire.remarks}
          onChange={(remarks) => updateTire({ remarks })}
        />
      </Typography>
      <Typography component="div" className={classes.uploadButton}>
        <Button
          variant="contained"
          color="primary"
          size="large"
...
RemarkField.propTypes = StringFieldPropTypes;
